#!/usr/bin/env python
"""
Pregenant 代谢组学数据完整分析脚本
直接在命令行运行，生成所有结果和图表

Usage:
    python scripts/run_preganent_full.py
...
def main():
    ...
    print(f"目标分布: {df['Pregent'].value_counts().to_dict()}")
    ...
    # 2. Benchmark 对比
    print_section("2. Benchmark 对比 (9种方法)")
    
    models = {
        'TabPFN': TabPFNClassifier.create_default_for_version(ModelVersion.V2, n_estimators=4, device='cpu'),
    ...
    tabpfn_model = TabPFNClassifier.create_default_for_version(ModelVersion.V2, n_estimators=4, device='cpu')
    tabpfn_model.fit(X_train, y_train)
    
    background = X_train.iloc[:min(20, len(X_train))]
    shap_sample = X.copy()  # 使用全部数据
    ...
    shap.summary_plot(shap_values_class1, shap_sample, feature_names=feature_names,
    ...
    for i, (feature_idx, y_pos) in enumerate(zip(top_indices, y_positions)):
        ...
        normed = (feature_vals - feature_vals.min()) / val_range if val_range > 0 else np.full_like(feature_vals, 0.5)
        colors = plt.cm.RdBu_r(normed)
        jitter = np.random.normal(0, 0.05, len(shap_vals))
        ax.scatter(shap_vals, [y_pos] * len(shap_vals) + jitter, c=colors, alpha=0.7, s=40,
